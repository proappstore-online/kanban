...
function SettingsIcon({ href }: { href: string }) {
  return (
    <a
      ...
      title="Settings"
      className="flex size-11 sm:size-8 shrink-0 items-center justify-center rounded-full text-[var(--muted)] hover:text-[var(--ink)] transition-colors"
    >
...
function AccountMenu({ user }: { user: User }) {
  return (
    <a
      href="#/profile"
      aria-label="Profile"
      className="flex size-11 sm:size-8 shrink-0 items-center justify-center rounded-full ring-2 ring-transparent hover:ring-[var(--line-strong)] transition-shadow"
    >
      {user.avatarUrl ? (
