 ...
 * cross-tenant SQL lockdown, so a regular signed-in user gets a 403 here —
 * that's fine: the schema is already migrated (a team member's visit applies
 * anything new), so swallow the 403 and continue. Every user-facing
 * read/write goes through registered actions (see lib/actions.ts), not raw
 * SQL, so those keep working regardless of this call's outcome.
 */
